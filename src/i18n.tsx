...
function translateRuntimeMessage(message: string, language: AppLanguage): string {
  ...
  const replacements: Array<[RegExp, string]> = [
    ...
    [/^正在分析其他歌曲，请先停止当前任务。$/, "Another analysis is already running. Stop it before starting a new one."],
    [/^分析进行中；请先停止任务再修改项目。$/, "Analysis is in progress. Stop it before editing the project."],
    [/^分析已停止。未完成的歌曲仍保留在列表中。$/, "Analysis stopped. Unfinished tracks remain in the list."],
    [/^分析已停止。已恢复重新分析前的结果。$/, "Analysis stopped. The previous analysis results were restored."],
